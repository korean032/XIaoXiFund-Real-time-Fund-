...
不仅支持**实时估值**监控，更集成了 **AI 智能识别**功能，只需上传持仓截图，即可自动识别并录入资产数据。借助 **Cloudflare KV** 或 **Upstash Redis**，您的数据将在云端安全同步，随时随地掌握财富动态。
...
- **☁️ 云端数据同步**：支持 **Cloudflare KV** 或 **Upstash Redis** 数据持久化，内置多用户隔离机制，多设备（手机/电脑）无缝同步。
